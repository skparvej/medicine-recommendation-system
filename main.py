...
# Assuming med[0] is the string like: "['Antifungal Cream', 'Fluconazole']"
    med_str = med[0]

# Remove brackets and split by comma
    med_list = med_str.strip("[]").split(",")

# Remove quotes and whitespace
    med_list = [item.strip().strip("'").strip('"') for item in med_list]
    ...
    die_list = die_str.strip("[]").split(",")
# Assuming die[0] is a string that looks like a list

    die_list = [item.strip().strip("'").strip('"') for item in die_list]
# Remove brackets and split by comma
